"""
Documentado por Luis Angel Leyva Castillo
"""
...
#Tabuladores para que las cosas queden facheritas
TAB_1 = '\t - '
TAB_2 = '\t\t - '
TAB_3 = '\t\t\t - '
TAB_4 = '\t\t\t\t - '
...
#Imprime los paquetes ipv4 dado el protocolo usado
...
#Imprime los paquetes ipv6 dado el protocolo usado
...
#Imprime las cabeceras del icmpv6
...
#Regresa el siguiente cabecera dado una
...
#Desempaqueta el cuadro de Ethernet
...
#Obtiene formato de la direccion MAC
def get_mac_addr(bytes_addr):
    bytes_str = map('{:02x}'.format, bytes_addr)
    mac_addr = ':'.join(bytes_str).upper()
    return mac_addr

#Desempaqueta lo recibido en ipv4
...
#Regresa la direccion ip de forma bonita
def ipv4(addr):
    return '.'.join(map(str, addr))

#Desempaqueta los paquetes ICMP
def icmp_packet(data):
    icmp_type, code, checksum = struct.unpack('! B B H', data[:4])
    return icmp_type, code, checksum, data[4:]

#Desempaqueta los paquetes TCP
def tcp_seg(data):
